    ...
    return response.text().then((body) => {
      ...
      if (!contentType.includes('application/json')) {
        if (response.redirected || response.status === 403 || response.status === 401) {
          throw new Error('Upload failed because the session is not authenticated or CSRF validation failed. Please reload the page and try again.');
        }
        throw new Error(`Upload failed with HTTP ${response.status}. The server returned HTML instead of JSON.`);
      }
      ...
      try {
        ...
        throw new Error('Upload failed because the server returned invalid JSON.');
      }

      if (!response.ok) {
        const message = data.error || (data.errors || []).join(', ') || `Upload failed with HTTP ${response.status}.`;
