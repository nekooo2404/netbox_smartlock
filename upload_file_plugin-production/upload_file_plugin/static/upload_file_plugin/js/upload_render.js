(function (window) {
  ...
  function render(uploadRoot, files, markChanged = true) {
    ...
    if (!files.length) {
      list.innerHTML = '<div class="list-group-item text-muted">Chưa có file đính kèm</div>';
    ...
    files.forEach((file, index) => {
      ...
      const fileName = utils.escapeHtml(file.file_name || file.name || 'File chưa đặt tên');
      const meta = file.id
        ? [utils.formatSize(file.size), 'Đã lưu']
        : [utils.formatSize(file.size), 'Chờ lưu'];
      item.innerHTML = `
        ...
        <button type="button" class="btn btn-sm btn-outline-danger" title="Xóa file" data-index="${index}">
