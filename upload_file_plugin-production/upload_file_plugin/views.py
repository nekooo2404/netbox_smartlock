...
def _image_type_error(file_name, file_type):
    name = os.path.splitext(file_name or "File chưa đặt tên")[0]
    suffix = f" ({file_type})" if file_type else ""
    return f"File '{name}'{suffix} phải là ảnh: {', '.join(VALID_MEDIA_TYPES)}."


def _file_size_error(file_name):
    name = os.path.splitext(file_name or "File chưa đặt tên")[0]
    return f"File '{name}' vượt quá giới hạn 25MB."
...
def _json_login_required(request):
    ...
    if request.user.is_authenticated:
        return None
    return JsonResponse(
        {"success": False, "errors": ["Bạn cần đăng nhập. Vui lòng tải lại trang và đăng nhập lại."]},
        status=401,
    )
    ...
    for uploaded in files:
        ...
        if is_file_valid and validate_enabled:
            ...
            if file_type not in allowed_file_types:
                validation_error_message = (
                    f"File '{name}' không thuộc các định dạng được phép: "
        ...
        try:
            ...
            errors.append(f"Không thể lưu file '{uploaded.name}': {exc}")
...
def delete_temp_file_view(request):
    ...
    try:
        ...
        if not file_name or not file_path:
            return JsonResponse({"success": False, "error": "Thiếu tên file hoặc đường dẫn"}, status=400)
        ...
        if file_path.startswith(media_url):
            ...
            return JsonResponse({"success": False, "error": "Đường dẫn file không hợp lệ"}, status=400)
        else:
        ...
        return JsonResponse({"success": False, "error": "File không tồn tại"}, status=404)
