...
class AssetGroup(OrganizationalModel):
    """Nhóm tài sản dùng chung cho Tài sản DCIM và SmartLock."""
    ...
    exclude_from_visualization = models.BooleanField(
        default=False,
...
class Asset(NetBoxModel):
    ...
    def get_edit_url(self):
        return reverse("plugins:netbox_smartlock:asset_edit", kwargs={"pk": self.pk})

    def get_delete_url(self):
        return reverse("plugins:netbox_smartlock:asset_delete", kwargs={"pk": self.pk})
    ...
    def delete(self, *args, **kwargs):
        delete_uploaded_files(self, model_name="asset")
        return super().delete(*args, **kwargs)
