...
class AccessRequestIntegrationTest(TestCase):
    ...
    def test_access_request_quick_search_matches_vietnamese_status_label(self):
        accepted_request = self.make_request(
            name="Accepted Search Request",
            status=self.AccessRequest.STATUS_ACCEPTED,
        )
        rejected_request = self.make_request(
            name="Rejected Search Request",
            status=self.AccessRequest.STATUS_REJECTED,
        )
        self.login()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, accepted_request.name)
        self.assertNotContains(response, rejected_request.name)
        self.assertContains(response, "Đã chấp nhận")
        self.assertNotIn(">Accepted<", response.content.decode())

    def test_access_request_person_quick_search_matches_vietnamese_status_labels(self):
        accepted_request = self.make_request(
            name="Person Status Search",
            status=self.AccessRequest.STATUS_ACCEPTED,
        )
        valid_in_person = self.make_person(
            accepted_request,
            identity_code="111122223335",
            full_name="Entry Approved Visitor",
            verify_status=self.AccessRequestPerson.VERIFY_VALID,
            access_status=self.AccessRequestPerson.ACCESS_IN,
        )
        invalid_person = self.make_person(
            accepted_request,
            identity_code="111122223336",
            full_name="Blocked Exit Visitor",
            verify_status=self.AccessRequestPerson.VERIFY_INVALID,
            access_status=self.AccessRequestPerson.ACCESS_OUT,
        )
        self.login()
        self.grant_object_permission(self.AccessRequest, ["view"])
        self.grant_object_permission(self.AccessRequestPerson, ["view"])
        ...
        valid_response = self.client.get(
            reverse("plugins:netbox_smartlock:accessrequestperson_list"),
            {"q": "Hợp lệ"},
        )
        ...
        self.assertContains(valid_response, valid_in_person.full_name)
        self.assertNotContains(valid_response, invalid_person.full_name)
        self.assertContains(valid_response, "Hợp lệ")
        self.assertNotIn(">Valid<", valid_response.content.decode())
        self.assertEqual(in_response.status_code, 200)
        self.assertContains(in_response, valid_in_person.full_name)
        self.assertNotContains(in_response, invalid_person.full_name)
        self.assertContains(in_response, "In")
    ...
    def test_access_request_changelog_pages_render(self):
        access_request = self.make_request(name="Changelog Detail")
        person = self.make_person(
            access_request,
            identity_code="969696969697",
            full_name="Changelog Person",
        )
        self.login()
        self.grant_object_permission(self.AccessRequest, ["view"])
        self.grant_object_permission(self.AccessRequestPerson, ["view"])
        ...
        request_response = self.client.get(
            reverse("plugins:netbox_smartlock:accessrequest_changelog", kwargs={"pk": access_request.pk})
        )
        person_response = self.client.get(
            reverse("plugins:netbox_smartlock:accessrequestperson_changelog", kwargs={"pk": person.pk})
        )

        self.assertEqual(request_response.status_code, 200, request_response.content.decode())
        self.assertEqual(person_response.status_code, 200, person_response.content.decode())
